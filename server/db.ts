...
pool.on('error', async (err) => {
  ...
  try {
    await pool.end();
    const newPool = createConnection();
...
export async function initializeDatabase() {
  try {
    ...
    if (routes.length === 0) {
      ...
      const route1Result = await pool.query(`
      ...
      const route2Result = await pool.query(`
        INSERT INTO routes (name, status, description, start_location, end_location, waypoints, frequency, created_by) 
    ...
    const assignmentsResult = await pool.query('SELECT * FROM assignments');
    const assignments = assignmentsResult.rows;

    if (assignments.length === 0) {
      console.log("No se encontraron asignaciones, creando asignaciones de ejemplo...");

      const driversResult = await pool.query("SELECT id FROM users WHERE role = 'driver'");
      const drivers = driversResult.rows;

      if (drivers.length > 0) {
        const routesDataResult = await pool.query("SELECT id FROM routes LIMIT 1");
        const vehiclesDataResult = await pool.query("SELECT id FROM vehicles WHERE status = 'active' LIMIT 1");
